...
* **Deployed auth uses the OAuth2 password grant, a dev-mode stand-in for SMART launch.** The live Railway build authenticates via OpenEMR's `oauth_password_grant` (a pre-registered, admin-enabled confidential client borrowing the clinician's identity) plus the break-glass path, because the production-correct **SMART EHR-launch `authorization_code`** flow above is scoped as future work (M0–M2 built the borrowed-identity data plane, not the browser launch). This does not change the diagram or the trust boundaries — the token is still a user-bound OAuth2 token presented to the same OAuth2/SMART-FHIR API; only the grant used to obtain it differs.
